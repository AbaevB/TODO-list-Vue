...
Цель — сделать приложение ещё удобнее: пользователь не должен каждый раз выбирать фильтр после перезагрузки.
...
- При старте приложения читаем сохранённый фильтр

- Если его нет — используем значение по умолчанию: all

- Поддерживается HMR: безопасно при разработке
...
- Реактивное отслеживание через watch

- Нет нужды в { deep: true } — значение примитивное (string)

- Автоматически сохраняется при каждом изменении

### 3. Установка фильтра (безопасно при HMR)
...
- Функция вызывается из TodoFilters.vue

- Избегает прямого присваивания — сохраняет реактивность

- Совместима с горячей заменой модулей (HMR)

### 4. Проверка результата

- [] При выборе фильтра он сохраняется в localStorage

- [] После перезагрузки страницы фильтр восстанавливается

- [] Нет ошибок в консоли

- [] Работает при активном HMR
